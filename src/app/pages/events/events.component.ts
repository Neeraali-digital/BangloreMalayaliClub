      ...
      <div class="container-custom">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div *ngFor="let event of filteredEvents" class="card overflow-hidden">
            <div class="h-48 bg-gradient-to-br from-kerala-gold to-kerala-green flex items-center justify-center">
              <span class="material-icons text-white text-6xl">{{ getEventIcon(event.category) }}</span>
            </div>
            <div class="p-6">
              <div class="flex items-center justify-between mb-2">
                <span class="bg-kerala-gold text-white text-xs px-2 py-1 rounded-full">{{ event.category }}</span>
                <span class="text-kerala-gold font-semibold">{{ event.price }}</span>
              </div>
              ...
              <div class="space-y-2 mb-4">
                <div class="flex items-center text-sm text-gray-500">
                  <span class="material-icons text-kerala-gold mr-2 text-sm">event</span>
                  {{ event.date }}
                </div>
                <div class="flex items-center text-sm text-gray-500">
                  <span class="material-icons text-kerala-gold mr-2 text-sm">access_time</span>
                  {{ event.time }}
                </div>
                <div class="flex items-center text-sm text-gray-500">
                  <span class="material-icons text-kerala-gold mr-2 text-sm">location_on</span>
                  {{ event.location }}
                </div>
              </div>
    ...
    <section class="section-padding bg-kerala-gold">
      <div class="container-custom text-center">
        ...
        <div class="max-w-md mx-auto flex gap-4">
          ...
          <button class="bg-white text-kerala-gold hover:bg-gray-100 font-semibold px-6 py-3 rounded-lg transition-colors duration-300">
            Subscribe
          </button>
        </div>
      </div>
